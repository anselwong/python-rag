...
def get_knowledge_bases() -> List[dict]:
    return knowledge.list_knowledge_bases()


@router.post("", response_model=KnowledgeBaseResponse, status_code=status.HTTP_201_CREATED, summary="Create a knowledge base")
def post_knowledge_base(payload: KnowledgeBaseCreate) -> dict:
    return knowledge.create_knowledge_base(payload.name.strip(), payload.description.strip())


@router.get("/{knowledge_base_id}/documents", response_model=List[DocumentResponse], summary="List documents")
def get_documents(knowledge_base_id: str) -> List[dict]:
...
async def post_document(knowledge_base_id: str, file: UploadFile = File(...)) -> dict:
    try:
